...
```{r}
plot_dir <- "/Users/soleymanitl/Documents/HRV/Test_Subjects_Analysis_Output"
output_dir <- "/Users/soleymanitl/Documents/HRV/Test_Subjects_Analysis_Output"
```

## Load HRV_Data_Loop_Test and MADRS_Sample

```{r}
#Here you can load the samples or your own data 
HRVData <- read.csv("/Users/soleymanitl/Documents/HRV/Test_Subjects_Physio_Output/HRV_Data_Loop_Test.csv")
head(HRV_Data_Loop_Test)

MADRS <- read.csv("/Users/soleymanitl/Documents/HRV/Data/MADRS_Sample.csv")
head(MADRS)
```
...
```{r}
HRVData <- merge(HRVData, MADRS[, c("Subject_number", "Session_date", "Timepoint", "Drug", "Infusion")], 
...
#### Load HRV_ConfoundingFactors_Sample

```{r}
#Load here HRV_ConfoundingFactors_Sample or your own file of confounding factors 
RD_CF <- read_excel("/Users/soleymanitl/Documents/HRV/Data/HRV_ConfoundingFactors_Sample.xlsx")
head(RD_CF)

HRV_MADRS$Age <- RD_CF$Age[match(HRV_MADRS$Subject_number, RD_CF$Subject_number)]
HRV_MADRS$Sex <- RD_CF$Sex[match(HRV_MADRS$Subject_number, RD_CF$Subject_number)]
HRV_MADRS$BMI <- RD_CF$BMI[match(HRV_MADRS$Subject_number, RD_CF$Subject_number)]
...
```{r}
MADRS_diff_HRV <- merge(HRVData2, HRV_MADRS_Result[c("Subject_number", "Session_date", "MADRS_Total_diff")], 
...
timepoint_levels <- c("Baseline" = "Infusion_Baseline", "Start Infusion" = "Infusion_Begining", "During Infusion" = "Infusion_During")
...
combined_plot <- ggplot(avg_data %>% filter(!is.na(avg_PPG_Rate_Mean) & Timepoint != "Entrance" & Timepoint != "Exit"), aes(x = factor(Timepoint, levels = timepoint_levels), y = avg_PPG_Rate_Mean, color = Drug, group = Drug)) +
  ...
  scale_x_discrete(labels = c("Infusion_Baseline" = "Baseline", "Infusion_Begining" = "Beginning of Infusion", "Infusion_During" = "End of Infusion")) +
...
print(combined_plot)
ggsave(filename = file.path(plot_dir, "HRV_averageHR_KET_PBO_combined.png"), plot = combined_plot, width = 12, height = 6, dpi = 300)
```
...
# Define the factor levels for Timepoint
timepoint_levels <- c("Baseline" = "Infusion_Baseline", "Start Infusion" = "Infusion_Beginning", "During Infusion" = "Infusion_End")
...
# Plot for both drugs overlapped
combined_plot <- ggplot(avg_data %>% filter(!is.na(avg_HRV_RMSSD) & Timepoint != "Entrance" & Timepoint != "Exit"), aes(x = factor(Timepoint, levels = timepoint_levels), y = avg_HRV_RMSSD, color = Drug, group = Drug)) +
  geom_line(size = 2, show.legend = TRUE) +  # Increased line size to 1.5
...
print(combined_plot)
ggsave(filename = file.path(plot_dir, "HRV_averageHRV_KET_PBO_combined.png"), plot = combined_plot, width = 12, height = 6, dpi = 300)
```
...
# Plot for Drug B or whatever drug is your placebo 
plot_drug_B <- ggplot(HRVData2 %>% filter(Drug == "B"), aes(x = factor(Timepoint), y = HRV_RMSSD, color = "PBO", group = 1)) +
  geom_line(size = 1) +
  labs(x = "Time point", y = "HRV (RMSSD)") + 
  scale_color_manual(values = c("PBO" = "#595959"), name = "Drug") +
  theme_light() +  # Applying classic theme
  theme(panel.grid = element_blank(), text = element_text(size = 28)) +  # Setting font size to 28
  coord_cartesian(clip = "off")  # Prevent clipping of long x-axis labels

print(plot_drug_B)
ggsave(filename = file.path(plot_dir, "Placebo_HRV_RMSSD.png"), plot = plot_drug_B, width = 10, height = 6, dpi = 300)
```

### 3.e. HRV Over Time for Poster

```{r}
# Create the ggplot
plot <- ggplot(HRVData2, aes(Timepoint, HRV_RMSSD, group = Subject_number)) +
  ...
  labs(x = "Time point", y = "HRV (RMSSD)") +
  scale_x_discrete(labels = c("Infusion_Baseline" = "Baseline", "Infusion_Beginning" = "Beginning\nof Infusion"), expand = c(0.4, 0.4)) +  # Add more space between time points
  theme_light() +  # Apply light theme
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), text = element_text(size = 28)) +  # Set font size to 28
...
### 3.f. Correlation Between Change in Mood Rating and HR Level at Baseline
